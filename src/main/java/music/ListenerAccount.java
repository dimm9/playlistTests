...
public class ListenerAccount extends Account {
    ...
    public Playlist createPlaylist(List<Integer> songIds) throws SQLException, NotEnoughCreditsException {
        ...
        for(var id: songIds) {
            if(!Persistence.hasSong(this.getId(), id)) {
                buySong(id);
            }
            ...
            if(optionalSong.isPresent())
                ...
                throw new NotEnoughCreditsException("Song with id " + id + " not found");
        }
        return playlist;
    }

    public void buySong(Integer id) throws SQLException, NotEnoughCreditsException {
        if(!Persistence.hasSong(this.getId(), id)) {
            if (this.getCredit() > 0) {
                Persistence.addSong(this.getId(), id);
                Persistence.addCredits(this.getId(), -1);
            }else if (this.getCredit() == 0) {
                throw new NotEnoughCreditsException("0 credits");
            }
        }
    }

    public int getCredit() throws SQLException {
        return Persistence.getCredits(this.getId());
    }
    public void addCredit(int amount) throws SQLException {
        Persistence.addCredits(this.getId(), amount);
    }
